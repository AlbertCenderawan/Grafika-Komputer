...
namespace ComputerGraphics.Class_Meeting.PERTEMUAN_06
{
    public partial class FormStatistics : Form
    {
        ...
        internal void LetsDraw()
        {
            ...
            DrawExtensionLine(pbStatistics, bitmapPicture_Statistics);

            DrawLineChart(pbStatistics, bitmapPicture_Statistics);

            DrawBarChart(pbStatistics, bitmapPicture_Statistics);
            DrawScatterChart(pbStatistics, bitmapPicture_Statistics);
        }
        ...
        private void DrawChartAttribute(PictureBox pictureBox, Bitmap bitmapPicture)
        {
            ...
            for (DataOrder = 1; DataOrder <= GetMaximumValue(); DataOrder++)
            {
        ...
        private void DrawExtensionLine(PictureBox pictureBox, Bitmap bitmapPicture)
        {
            int DataOrder;
            int HorizontalAxisPoint_Start, HorizontalAxisPoint_End;
            int VerticalAxisPoint_Start, VerticalAxisPoint_End;
        ...
        private void DrawScatterChart(PictureBox pictureBox, Bitmap bitmapPicture)
        {
            int DataOrder;
            int Radius;
            ...
            for (DataOrder = 0; DataOrder < dataTableCount; DataOrder++)
            {
                DrawCircle(pbStatistics, bitmapPicture_Statistics, ChartPoints[DataOrder].X, ChartPoints[DataOrder].Y, Radius, 0x00, 0x00, 0x00);
                //FillObjectArea(pbStatistics, bitmapPicture, ChartPoints[DataOrder].X, ChartPoints[DataOrder].Y, 0xff, 0x00, 0x00);
            }
        }

        private void FillObjectArea(PictureBox pictureBox, Bitmap bitmapPicture, int xPoint, int yPoint, byte RedColor, byte GreenColor, byte BlueColor)
        {
            int xOrder, yOrder;
            Color backgroundPixel, pointPixel;
            byte[] pointColor = new byte[3];
            byte[] backgroundPixelColor = new byte[3];

            bool xIsLooping, yIsLooping;

            // Ambil warna di titik start sebagai patokan warna
            // Warna di manapun harus dipatok dari warna titik start
            backgroundPixel = bitmapPicture.GetPixel(xPoint, yPoint);

            backgroundPixelColor[0] = backgroundPixel.R;
            backgroundPixelColor[1] = backgroundPixel.G;
            backgroundPixelColor[2] = backgroundPixel.B;

            xOrder = xPoint;
            xIsLooping = true;

            // Cek dari KUADRAN 2 (x ke KIRI, y ke ATAS)
            while (xIsLooping && xOrder > 0) // x looping terus sampai ketemu warna yang beda
            {
                pointPixel = bitmapPicture.GetPixel(xOrder, yPoint);
                pointColor[0] = pointPixel.R;
                pointColor[1] = pointPixel.G;
                pointColor[2] = pointPixel.B;

                // Cek apakah warna pada point yang ditunjuk = warna titik patokan
                if (RGBColorsAreSame(backgroundPixelColor, pointColor))
                {
                    yOrder = yPoint;
                    yIsLooping = true;

                    while (yIsLooping && yOrder > 0) // y looping terus sampai ketemu warna yang beda
                    {
                        pointPixel = bitmapPicture.GetPixel(xOrder, yOrder);
                        pointColor[0] = pointPixel.R;
                        pointColor[1] = pointPixel.G;
                        pointColor[2] = pointPixel.B;

                        // Cek apakah warna pada point yang ditunjuk = warna titik patokan
                        if (RGBColorsAreSame(pointColor, backgroundPixelColor))
                        {
                            int R = Convert.ToInt32(RedColor);
                            int G = Convert.ToInt32(GreenColor);
                            int B = Convert.ToInt32(BlueColor);
                        ...
                        else yIsLooping = false; // kalau ketemu warna yang beda, looping dihentikan

                        yOrder--;
                    }
                }
                else xIsLooping = false; // kalau ketemu warna yang beda, looping dihentikan

                xOrder--;
            }

            xOrder = xPoint + 1; // titik patokan + 1 (biar tidak menimpa titik yang diberi warna)
            xIsLooping = true;

            // Cek dari KUADRAN 1 (x ke KANAN, y ke ATAS)
            while (xIsLooping && xOrder < bitmapPicture.Width) // x looping terus sampai ketemu warna yang beda
            {
                pointPixel = bitmapPicture.GetPixel(xOrder, yPoint);
                pointColor[0] = pointPixel.R;
                pointColor[1] = pointPixel.G;
                ...
                if (RGBColorsAreSame(backgroundPixelColor, pointColor))
                {
                    yOrder = yPoint; // titik patokan (biar tidak menimpa titik yang diberi warna)
                    yIsLooping = true;
            ...
            xOrder = xPoint; // titik patokan - 1 (biar tidak menimpa titik yang diberi warna)
            yPoint++;
            xIsLooping = true;

            // Cek dari KUADRAN 3 (x ke KIRI, y ke BAWAH)
            while (xIsLooping && xOrder > 0) // x looping terus sampai ketemu warna yang beda
            {
                pointPixel = bitmapPicture.GetPixel(xOrder, yPoint);
                pointColor[0] = pointPixel.R;
                ...
                if (RGBColorsAreSame(backgroundPixelColor, pointColor))
                {
                    yOrder = yPoint; // titik patokan - 1 (biar tidak menimpa titik yang diberi warna)
                    yIsLooping = true;

                    while (yIsLooping && yOrder < bitmapPicture.Height) // y looping terus sampai ketemu warna yang beda
                    {
                        pointPixel = bitmapPicture.GetPixel(xOrder, yOrder);
                        pointColor[0] = pointPixel.R;
                        pointColor[1] = pointPixel.G;
            ...
            // Cek dari KUADRAN 4 (x ke KANAN, y ke BAWAH)
            while (xIsLooping && xOrder < bitmapPicture.Width) // x looping terus sampai ketemu warna yang beda
            {
                pointPixel = bitmapPicture.GetPixel(xOrder, yPoint);
                pointColor[0] = pointPixel.R;
        ...
        private bool RGBColorsAreSame(byte[] backgroundPixelColor, byte[] pointColor)
        {
            int sameComponentColor = 0;
            byte colorIdx;

            for (colorIdx = 0; colorIdx < 3; colorIdx++)
            {
                if (backgroundPixelColor[colorIdx].Equals(pointColor[colorIdx])) sameComponentColor++;
            }

            if (sameComponentColor == 3) return true;
            else return false;
        }
